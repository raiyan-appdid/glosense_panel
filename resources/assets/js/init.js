...
function setTheme(data) {
  ...
  document.cookie =
    type[1] === "feather-moon"
      ? "theme=dark; expires=Thu, 01 Jan 2026 00:00:00 UTC; path=/"
      : "theme=light; expires=Thu, 01 Jan 2026 00:00:00 UTC; path=/";
}
...
const reboundForm = async function ({
  selector = null,
  data = null,
  type = "POST",
  route = null,
    ...
    success: function (response) {
      ...
      if (selector !== null) {
        ...
        if (reset) {
          ...
          $(
            `form#${$(selector).attr("id")} select, form input[type=checkbox]`
          ).trigger("change");
...
$(document).on("click", ".view-on-click", function () {
  try {
    Swal.fire({
      html:
        '<img class="s-image" src="' + $(this).attr("src") + '"  alt="image"/>',
...
$(document).on("click", ".view-modal-data", function () {
  ...
  if ($(".modal").length == 0) {
    ...
    snb(
      "warning",
      "Modal not found",
      `The modal with id "${modal}" was not found.`
    );
    $(btn).attr("disabled", false).html(btn_text);
    return false;
  }
...
function noCallback() {
  snb(
    "error",
    "Callback undefined",
    "No callback function defined for edit action."
  );
}
