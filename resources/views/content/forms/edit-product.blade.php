...
@section('content')
    <section>
        <x-card title="Edit Product">
            <x-form successCallback="redirect" id="edit-product" method="POST" class="" :route="route('admin.products.update')">
                <x-input name="id" type="hidden" value="{{ $product->id }}" />
                <div class="col-lg-4 col-12">
                    <x-input name="title" value="{{ $product->title }}" />
                </div>
                <div class="col-lg-6 col-12">
                    <x-input name="video_link" :required="false" value="{{ $product->video_link }}" type="url" />
                </div>
                {{-- <div class="col-lg-3 col-12">
                    <x-input-file name="images" :multiple="true" :required="false" />
                </div> --}}
                {{-- <div class="col-lg-1 col-12 mt-1">
                    <div class="avatar-group">
                        @foreach ($product->images as $img)
                            <x-input name="img_id[]" type="hidden" value="{{ $img->id }}" />
                            <div class="avatar avatar-lg pull-up">
                                <img class="view-on-click" src="{{ asset($img->image) }}" alt="Avatar" height="32"
                                    width="32">
                            </div>
                        @endforeach
                    </div>
                </div> --}}
                <x-image-uploader :preview="$product->images" name="images" id="images" />
                <div class="col-lg-4 col-12">
                ...
                <div class="col-lg-4 col-12">
                    <x-select name="unit_id" :select="$product->unit_id" :options="$units" label="Unit" />
                </div>
                <div class="col-lg-4 col-12">
                ...
                <div class="col-lg-12 col-12">
                    <x-editor type="textarea" id="edit_how_to_take" name="how_to_take" label="How to take" />
                </div>
            </x-form>
        </x-card>
    </section>
