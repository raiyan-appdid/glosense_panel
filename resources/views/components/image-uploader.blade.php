    ...
    <div id="{{ $id }}" class="row text-center">
        @if (!empty($preview))
            @foreach ($preview as $p)
                <div class="{{ $class }}  imgUp">
                    <div class="imagePreview"
                        style="background:url('{{ asset($p->image) ?? (asset($p['image']) ?? asset($p)) }}');background-size: cover;background-repeat: no-repeat;">
                    </div>
                    <label class="btn btn-primary">
                        Upload
                        <input type="hidden" value="{{ $p->id ?? $p['id'] }}" name="{{ $name }}_id[]">
                        <input type="file" value="{{ $p->image }}"
                            name="{{ $name }}[{{ $p->id ?? $p['id'] }}]" class="uploadFile img" accept="image/*"
                            style="width: 0px;height: 0px;overflow: hidden;">
                    </label>
                    <span class="del-img" data-callback="deleteElement"
                        data-delete="{{ route('admin.products.image_destroy', $p->id) }}"><svg aria-hidden="true"
                            focusable="false" data-prefix="fa-regular" data-icon="xmark"
                            class="svg-inline--fa fa-xmark fa-w-10" role="img" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 320 512">
                            <path
